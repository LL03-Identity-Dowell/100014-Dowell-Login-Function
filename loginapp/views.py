...
def RegisterPage(request):
            ...
            try:
                ...
                for obj in filtered_accts:
                    if email == obj.email and role1 == obj.role:
                        filtered_accts = Account.objects.filter(email=email).update(password = make_password(password1),first_name = first,last_name = last,email = email,phonecode=phonecode,phone = phone,profile_image=img)
    ...
    return render(request,'register_v2.html',{'form': form, 'title':'reqister here','country_resp':final_list,'org':orgs,'type':type1})
